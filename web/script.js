...
(function () {
  ...
  function makeColumnHeaders(t, result) {
    for (var i = 0; i <= result.servers.length; i++) t.appendChild(document.createElement("colgroup"));
    ...
    for (var i = 0; i < result.servers.length; i++) {
      ...
      if (result.hasOwnProperty("urls")) makeClickable(cell, result.urls[result.servers[i]]);
  ...
  function makeRowHeader(tbody, result, i) {
    ...
    if (result.hasOwnProperty("urls")) makeClickable(cell, result.urls[result.clients[i]]);
    ...
    for (var i = 0; i < result.clients.length; i++) {
      ...
      for (var j = 0; j < result.servers.length; j++) {
        ...
        for (var k = 0; k < res.length; k++) {
          ...
          if (measurement.result === "succeeded") link.innerHTML += ": " + measurement.details;
  ...
  function dateToString(date) {
    return (
      ...
      " " +
      date.toLocaleTimeString("en-US", { timeZone: "UTC", timeZoneName: "short" })
    );
  }
  ...
  function setButtonState() {
    ...
    $("#test :button").each((i, e) => {
      ...
      Object.keys(count).map((c) => {
        ...
        if (e.classList.contains("active") === false) b.className += " disabled";
        b.id = e.id + "-" + c;
  ...
  function clickButton(e) {
    ...
    var params = new URLSearchParams(history.state ? history.state.path : window.location.search);
    if (params.has(type) && params.get(type)) map[type] = params.get(type).split(",");
    else
      map[type] = $("#" + type + " :button")
        .get()
    ...
    if (map[type].length === $("#" + type + " :button").length) params.delete(type);

    const comp = decodeURIComponent(params.toString());
    var refresh =
      window.location.protocol + "//" + window.location.host + window.location.pathname + (comp ? "?" + comp : "");
    ...
    if (result.hasOwnProperty("tests"))
      $("#test").append(
        Object.keys(result.tests).map((e) =>
          makeButton("test", e, makeTooltip(result.tests[e].name, result.tests[e].desc))
        )
      );
    else {
      // TODO: this else can eventually be removed, when all past runs have the test descriptions in the json
      const tcases = result.results
        .concat(result.measurements)
        .flat()
        .map((x) => [x.abbr, x.name])
        .filter((e, i, a) => a.map((x) => x[0]).indexOf(e[0]) === i);
  ...
  function load(dir) {
    ...
    xhr.onreadystatechange = function () {
      ...
      var refresh =
        window.location.protocol + "//" + window.location.host + window.location.pathname + "?run=" + selectedRun;
      window.history.pushState(null, null, refresh);
      process(result);
